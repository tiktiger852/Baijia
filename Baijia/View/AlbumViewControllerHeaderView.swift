...
class AlbumViewControllerHeaderView: UIView {
    ...
    override func awakeFromNib() {
        ...
        self.mask2View.backgroundColor = RGBA(40, 40, 40, 0.5)
    ...
    var info: AlbumIntro? {
        didSet {
            self.coverImageView.kf_setImageWithURL(NSURL(string: self.info!.coverLarge)!, placeholderImage: nil, optionsInfo: nil) { (image, error, cacheType, imageURL) -> () in
                if let image1 = image {
                    self.coverImageView.image = image1.applyBlurWithRadius(20.0, tintColor: RGBA(0, 0, 0, 0), saturationDeltaFactor: 1.0, maskImage: nil)
            ...
            for (key, value) in tags.enumerate() {
                ...
                tagButton.setBackgroundImage(UIImage.fromColor(RGB(210, 210, 210)), forState: .Highlighted)
